...
from conv_deconv_v2 import conv_fn, loss_fn, sensor_noise, feat_extract, wiener, cal_psnr
from torch.utils.tensorboard import SummaryWriter

"""
v2: 不仅有FE, 还含有FP和DE的网络
"""
...
def train(args):
    ...
    for i in range(args.epochs):
        ...
        for j, img in enumerate(trainloader):
            ...
            # deconv_result = nn(deconv_result, psf)                                                # (b,3,810,810)
            # nn(wiener)
            blur = blur.reshape(blur.size(0), -1, blur.size(-2), blur.size(-1))
            deconv_result = nn(blur, psf)
        ...
        if (i % args.log_freq == 0):
            for j, img in enumerate(evalloader):
                with torch.no_grad():
                    ...
                    loss_val = loss_fn(deconv_result, img, params)
                    loss_val_epoch += loss_val * img.size(0)
                    psnr_val_epoch += cal_psnr(img, deconv_result) * img.size(0)
            loss_val_epoch /= len(evalset)
            psnr_val_epoch /= len(evalset)
            writer.add_scalar('eval loss vs epochs', loss_val_epoch, i)
            writer.add_scalar('psnr vs epochs', psnr_val_epoch, i)
